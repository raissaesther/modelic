...
if (!function_exists('qode_scripts')) {
	...
	function qode_scripts() {
		...
		if (isset($_SESSION['qode_mondelic_page_transitions'])) {
			$qode_animation = $_SESSION['qode_mondelic_page_transitions'];
		}
...
if (!function_exists('qode_generate_dynamic_css_and_js')){
	...
	function qode_generate_dynamic_css_and_js() {
		...
		$qode_options = get_option('qode_options_mondelic');
		...
		if($pagenow === 'admin.php' && $is_theme_options_page) {
			if(!qode_assets_folders_writable()) { ?>
				<div class="error">
					...
					For further instructions, please refer to our <a target="_blank" href="http://demo.select-themes.com/mondelic-help/#!/getting_started">documentation</a>.', 'qode'); ?></p>
				</div>
			<?php }
		}
	}
